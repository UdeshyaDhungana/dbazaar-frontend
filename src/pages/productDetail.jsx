import { Badge, Box, FormLabel, Heading, Icon, Modal, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay, Switch, Text, Tooltip, useDisclosure, useToast } from '@chakra-ui/react';
...
import { deleteProduct, getSingleProuct, toggleVisibility } from '../services/productService';
    ...
    return (
        <>
            <div className="grid my-10 md:grid-cols-2">
                ...
                <Box>
                    <div>
                        <Heading marginBottom={"3"} size={"lg"}>{label}  {!visible && <Badge>Unlisted</Badge>}</Heading>
                        <FormLabel fontWeight={"bold"} htmlFor='visibility'>Visibility: </FormLabel>
                        {user && user.user_id === productOwnerId &&
                            <>
                                <Switch marginRight={"3"} id='visibility' onChange={handleToggle} isChecked={visible} />
                                <Tooltip label='Toggling visibility will delete all transfers and bids related to the product' fontSize='md'>
